...
export const Inter = ({ title, subtitle, text, src, isRight, icon }: PropsInter) => {
    ...
    return (
        <div className={`flex ${!isRight && 'flex-row-reverse'} w-full`}>
            <div ref={element} className={`w-[50%] text-(--color-palette-one)/80  pt-[3.5rem] pb-[3.5rem] ${isRight ? 'text-right pr-4 lg:pr-[3rem]' : 'text-left pl-4 lg:pl-[3rem]'} flex flex-col gap-2`}>
            ...
            <div className={`relative w-[50%] flex items-center border-(--color-palette-one)/10 ${isRight ? 'justify-self-start border-l' : 'justify-end border-r'}  ${icon && 'border-t'}`}>
