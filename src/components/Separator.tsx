export const Separator = ({ src = '', title, element }: { src?: string, title: string, element?: JSX.Element }) => (
    <>
        ...
        <span className='border-t border-b border-(--color-palette-one)/30 p-[.5rem_1.5rem] mb-7'>
            <p className='font-[Lora] text-xl bg-gradient-to-r from-(--color-palette-one) via-(--color-palette-one) to-(--color-palette-one) text-transparent bg-clip-text sm:text-3xl'>{title}</p>
        </span>
    </>
);
